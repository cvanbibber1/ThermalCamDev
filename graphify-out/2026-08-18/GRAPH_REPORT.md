...
## Corpus Check
- 59 files · ~48,159 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 548 nodes · 811 edges · 65 communities (61 shown, 4 thin omitted)
- Extraction: 85% EXTRACTED · 15% INFERRED · 0% AMBIGUOUS · INFERRED: 121 edges (avg confidence: 0.82)
...
- `test_wire_round_trip_and_crc_rejection()` --calls--> `wire_encode()`  [INFERRED]
  test/test_core/test_main.c → src/protocol/wire_protocol.c
- `test_wire_round_trip_and_crc_rejection()` --calls--> `wire_decode()`  [INFERRED]
  test/test_core/test_main.c → src/protocol/wire_protocol.c
...
Cohesion: 0.11
Nodes (36): board_lepton_cs(), board_lepton_power(), board_lepton_reset(), board_lepton_spi_clock_enable(), board_lepton_spi_clock_hold(), HAL_ADC_ConvCpltCallback(), HAL_ADC_ConvHalfCpltCallback(), lepton_capture_status_t (+28 more)
...
Cohesion: 0.23
Nodes (28): board_unique_id_word(), lepton_capture_latest_frame(), append_u32(), assign_bus_address(), begin_response(), cci_get(), cci_run(), cci_set() (+20 more)

### Community 7 - "Dosimeter ADC"
Cohesion: 0.07
Nodes (30): board_reset_cause(), board_rs485_de(), dosimeter_snapshot_t, dosimeter_get_snapshot(), dosimeter_init(), dosimeter_task(), integer_sqrt(), process_half() (+22 more)

### Community 8 - "VoSPI and CRC"
Cohesion: 0.17
Nodes (14): packet_is_discard(), vospi_latest_frame(), vospi_packet_crc(), vospi_parse_segment(), vospi_parser_init(), crc16_ccitt(), make_segment(), test_crc_known_vectors() (+6 more)
...
Cohesion: 0.13
Nodes (14): 1. Prove the Lepton clocks and SPI waveform electrically, 2. Replace the large pipeline temporarily with a minimal capture diagnostic, 3. Restore camera configuration and compact capture, 4. Prove USB camera delivery, 5. Finish RS422/UART last, Completed and verified, Current flashed firmware state, Current hardware and host state (+6 more)
...
Cohesion: 0.18
Nodes (10): Command and telemetry channel, UVC and CDC ACM Composite Device, Data-Only Self-Powered USB, Device composition, USB Endpoint and FIFO Layout, USB hardware constraints, USB interface, UVC video mode (+2 more)
...
Cohesion: 0.29
Nodes (8): Immutable Frame Broker, Transport-Independent Nonblocking Capture, Multidrop Video Capacity Budget, Bounded Chunked Frame Transfer, Drop-Oldest UVC Backpressure Policy, Radiometric UVC Y16 Stream, D-006 Capture Never Blocks on Transport, UVC Y16 Host Compatibility Risk
...
## Knowledge Gaps
- **185 isolated node(s):** `recommendations`, `unwantedRecommendations`, `I2C_HandleTypeDef`, `SPI_HandleTypeDef`, `ADC_HandleTypeDef` (+180 more)
...
- **Why does `health_increment()` connect `Lepton Capture State Machine` to `Board Clock and Peripherals`, `Dosimeter ADC`?**
