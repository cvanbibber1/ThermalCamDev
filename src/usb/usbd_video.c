...
/**
  * @brief  Format index the host committed to, 1 for YUY2 or 2 for Y16.
  *         The host writes it into the commit control, which SET_CUR receives
  *         directly, so no extra bookkeeping is required.
  */
uint8_t USBD_VIDEO_GetCommittedFormat(void)
{
  return video_Commit_Control.bFormatIndex;
}
